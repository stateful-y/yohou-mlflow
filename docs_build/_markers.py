...
def _build_api_table_html(project_root, prefix):
    ...
    for module_name, members in scans:
        module_label = _qualified_name(module_name, "").rstrip(".") or "yohou_mlflow"
...
def _get_notebook_api_usage(project_root):
    ...
    Scans example notebooks for ``from yohou_mlflow.* import …``
    ...
    for notebook in sorted(examples_dir.rglob("*.py")):
        ...
        if declared is not None:
            ...
            # Extract names imported from yohou_mlflow.*
            imported_names = set()
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom) and node.module and node.module.startswith("yohou_mlflow"):
...
def _build_module_toc(project_root, current_src_path=None, prefix=None):
    ...
    for mod in modules:
        ...
        entry = {
            "title": f"yohou_mlflow.{mod['module_name']}",
            "url": page_url,
            "active": active,
            "children": [],
        }
