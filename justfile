# Available commands for Yohou-MLflow
...
test:
    uv run pytest tests src/yohou_mlflow --doctest-modules --doctest-continue-on-failure -n auto -v
...
test-cov:
    uv run pytest --cov=yohou_mlflow --cov-report=html --cov-report=term -n auto

# Run docstring examples
test-docstrings:
    uv run pytest --doctest-modules --doctest-continue-on-failure --no-cov src/yohou_mlflow
